...
def process_file(uploaded_file):
    ...
    if uploaded_file.type == "application/pdf":
        ...
        for page_num in range(min(len(doc), 5)): 
            ...
            # UPDATED: Higher Resolution (3,3) to help AI see small numbers better
                ...
                with tab1:
                    if "shopping_list" in data:
                        ...
                        if hd_final < lowes_final:
                            diff = lowes_final - hd_final
                            with col1:
                                st.success(f"🏆 HOME DEPOT: ${hd_final:.2f}")
                                st.caption(f"Subtotal: ${hd_subtotal:.2f} | Tax ({tax_rate}%): ${hd_tax:.2f}")
                            with col2:
                                st.error(f"Lowe's: ${lowes_final:.2f}")
                                st.caption(f"Subtotal: ${lowes_subtotal:.2f} | Tax ({tax_rate}%): ${lowes_tax:.2f}")
                        else:
                            diff = hd_final - lowes_final
                            with col1:
                        ...
                        * **AI Estimation:** This list is generated by Artificial Intelligence. While often accurate, it can make errors in counting or identifying materials. **Always verify** the Shopping List against your original plan before buying.
                        * **Pricing:** Prices are estimates based on national averages and may vary by store location and season. 
                        * **Waste:** Estimates do not account for cut waste unless specified. We recommend buying 10% extra lumber.
                        """)
